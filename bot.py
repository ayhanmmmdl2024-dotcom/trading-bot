"""
╔══════════════════════════════════════════════════════════════════════════╗
║           UNIVERSAL SNIPER v12.0 — İNSTİTUTİONAL ARXİTEKTURA          ║
╠══════════════════════════════════════════════════════════════════════════╣
...
╠══════════════════════════════════════════════════════════════════════════╣
║  v10-dan saxlananlar:                                                   ║
║   ✅ 500+ aktiv (Binance 400+, Forex 30, Əmtəə 18, SP500, Avropa)     ║
║   ✅ 5M→15M→1H→4H→1D MTF analiz                                       ║
║   ✅ OB, FVG, Sweep, Pullback, Breakout, Wyckoff                       ║
║   ✅ ML pipeline (GBM + StandardScaler)                                 ║
║   ✅ WFO, backtest, Kelly risk engine                                   ║
║   ✅ Telegram queue, DB queue, TTL cache                                ║
╚══════════════════════════════════════════════════════════════════════════╝
"""
...
MIN_ULDUZ        = float(_env("MIN_ULDUZ", "2.4"))
...
def mtf_trend_yoxla(frames):
    ...
    if long_say >= 2:      # 3 → 2 (yumşaldıldı)
        return 'LONG', long_say
    elif short_say >= 2:   # 3 → 2
        return 'SHORT', short_say
    return 'NEYTRAL', 0
...
def siqnal_gonder(sym: str, res: dict, sl_lbl='NEYTRAL',
    ...
    # ML Filter (tam aktiv)
    if not ml_filter(res):
        print(f"[ML] {sym} siqnalı ML tərəfindən rədd edildi.")
        return
    ...
    msg=(
        f"🌐 <b>UNIVERSAL SNIPER v11.0 — {res['type']}</b>\n\n"
        f"💎 <b>Aktiv:</b> #{clean}\n"
        f"{'─'*28}\n"
...
if __name__ == '__main__':
    log.info("╔══════════════════════════════════════════════╗")
    log.info("║   UNIVERSAL SNIPER v12.0 — İNSTİTUTİONAL   ║")
    log.info("║   AsyncIO + WebSocket + Event-Driven        ║")
    log.info("╚══════════════════════════════════════════════╝")
    ...
    telegram_gonder(
        "🚀 <b>UNIVERSAL SNIPER v12.0 — İNSTİTUTİONAL</b>\n\n"
        "✅ AsyncIO + Native WebSocket\n"
        "✅ Event-driven (while True yoxdur)\n"
        "✅ Bounded semaphore (thread-safe)\n"
        ...
        "✅ Risk Engine v2 (Kelly+portfolio)\n"
        "✅ Realistic backtest (fee+slippage)\n"
        "✅ Latency tracker (p50/p95/p99)\n"
        "✅ Prometheus + Grafana :9090\n"
        "✅ <b>[NEW] Market Regime State Machine</b> (hysteresis 3-bar)\n"
        "✅ <b>[NEW] Liquidity Filter</b> (real-time spread + z-score)\n"
        "✅ <b>[NEW] Chaos Testing</b> (/chaos komandasından)\n"
        "✅ <b>[NEW] Backpressure Queue</b> (10K msg/s flood qoruması)\n\n"
        "📊 500+ aktiv | 5M→15M→1H→4H→1D\n"
        "⭐ Min skor: 3.0 | 🎯 Min RR: 1:2\n\n"
...
def analiz_et(frames: dict, asset_type='crypto', symbol='', ref_idx=-2) -> Optional[dict]:
    ...
    try:
        ...
        # YUMŞALDILMIŞ VERSİYA
        if mtf_yon == 'NEYTRAL':
            mtf_yon = 'LONG' if rsi1h < 52 else 'SHORT'   # RSI-yə görə default yön
            mtf_skor = 1
